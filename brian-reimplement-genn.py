...
class SNN():
    def __init__(self):
        ...
        self.test_steps = 5
        self.global_clock = Clock(self.time_step * ms,name="defaultclock")

        self.rates = zeros(self.input_layer_size)
        # input neurons
        inp = NeuronGroup(self.input_layer_size, 'v:1', method='exact', threshold='v>=1',
                          reset='v=0',name="input",clock=self.global_clock)
        def update_volt():
            inp.v += self.rates
        # network_op = NetworkOperation(update_volt, dt=4.0 * ms)
        network_op = NetworkOperation(update_volt,clock=self.global_clock)
        # hidden neurons
        hidden = NeuronGroup(self.hidden_layer_size, self.eqs, threshold="v>cita_h",
                             reset='v=reset', method="exact",name="hidden",clock=self.global_clock)
        hidden.cita_h = self.cita_h
        hidden.reset = self.reset
        output = NeuronGroup(self.output_layer_size, self.eqs, threshold='v>cita_o',
                             reset='v=reset', method='exact',name="output",clock=self.global_clock)
        output.cita_o = self.cita_o
        output.reset = self.reset


        conn_ih = Synapses(inp, hidden, model='w:1', on_pre='v_post += w',name="conn_ih",clock=self.global_clock)
        conn_ih.connect(p=1)
        conn_ih.w = np.random.randn(self.input_layer_size*self.hidden_layer_size)
        conn_ho = Synapses(hidden, output, model='w:1', on_pre='v_post += w',name="conn_ho",clock=self.global_clock)
        conn_ho.connect(p=1)
